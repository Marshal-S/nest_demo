...
import {
    ...
    ArticleQueryDto,
    ArticleUpdateDto,
} from './dto/req-article.dto';
import { InjectRepository } from '@nestjs/typeorm';
import { Article } from './entities/article.entity';
import { And, Between, IsNull, MoreThan, Not, Repository } from 'typeorm';
...
export class ArticleService {
    ...
    //创建、加入专栏
    async create(body: ArticleCreateDto, user: User) {
    ...
    //更新
    async update(body: ArticleUpdateDto, user: User) {
    ...
    //软删除
    async remove(body: ArticleIdDto, user: User) {
    ...
    //删除
    async delete(body: ArticleIdDto, user: User) {
        let article = await this.articleRepository.findOneBy({
            id: body.id,
        });
        if (!article) {
    ...
    //查看文章
    async getArticle(body: ArticleIdDto, user: User) {
        let article = user
    ...
    async getArticleEx(body: ArticleIdDto, user: User) {
    ...
    //收藏文章
    async collect(body: ArticleCollectDto, user: User) {
        let article = await this.articleRepository.findOne({
            where: {
                id: body.id,
                isDelete: false,
    ...
    async collectEx(body: ArticleCollectDto, user: User) {
        let article = await this.articleRepository.findOne({
            where: {
                id: body.id,
                isDelete: false,
    ...
    async query(body: ArticleQueryDto, user: User) {
        ...
        let articles = await this.articleRepository
            ...
            .andWhere(
                'article.title LIKE :name OR article.desc LIKE :name OR article.content LIKE :name',
                {
                    name: `%${body.name}%`,
                },
            )
    ...
    async subQueryByAge() {
        const articles = await this.articleRepository
            .createQueryBuilder('article')
            .where(
                'EXISTS (select * from user where user.id=article.userId AND user.age is not null AND user.age>25)',
            )
          .getMany();
        // const articles = await this.articleRepository
        //     .createQueryBuilder('article')
        //     .leftJoin('article.user', 'user')
        //     .where('user.age is not null AND user.age>25')
      //     .getMany();
      
        // const articles = await this.articleRepository.find({
        //     where: {
        //         user: {
        //             age: And(Not(IsNull()), MoreThan(20)),
        //         },
        //     },
        // });
        return ResponseData.ok(articles);

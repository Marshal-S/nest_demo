...
import { CustomConfigType } from './file.module';
...
export class FileService {
    constructor(
        ...
        @Inject('file_custom') private customServce: CustomConfigType,
    ) {}

    async upload(mFile: Express.Multer.File) {
        if (!mFile) {
            return ResponseData.fail('请选择文件');
        }
        let file = new File();
        //解决乱码问题
        file.originalname = Buffer.from(mFile.originalname, 'latin1').toString(
            'utf8',
        );
    ...
    async uploadMinioEx(mFile: Express.Multer.File) {
        if (!mFile) {
            return ResponseData.fail('请选择文件');
        }
        let url = null;
    ...
    async testUpload() {
        return new Promise((resolve, reject) => {
            ...
            uploadByFileHandle(
                'public/4.mp4',
                async (err: Error, chunk: ChunkInfo) => {
                    if (err) {
                    ...
                    if (chunk.isCompleted) {
                        const filename = new Date().getTime().toString();
                        await this.minioService.compostObject(
                            filename,
                            filenames,
                        );
                        console.log('成功了');
    ...
    async getAllFiles() {
        return ResponseData.ok(await this.fileRepository.find());
    }
}
